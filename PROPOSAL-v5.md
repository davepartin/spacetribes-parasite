...
| Fog of war | This round's host only | Unchanged |
...
| **Heal scar** | 3⚡ | Remove one Scar die |
| **Section reroll** | 1🎲 or 1⚡ | Reroll Attack, Mod, or Scar section. **Spends reroll tokens first, then energy** |
...
New face set (was Energy / % / Base / Points / Scar / Scar):
...
| **Scar** ×1 | — | Lock it → grab a Scar die |
...
Dropping to **one scar face** is intentional: Poison and Outbreak still inject scars, and the lone scar face sitting next to Points makes greed-locking scarier.

⚠ Permanent income is the classic sim-before-ship mechanic. Run 200 games; watch for round-3 Energy claims deciding round-13.

### Scar dice — new faces

Redesigned from 3 blanks + 3 hurts to **one blank, five hurts** — and the color faces scale with your own build, so scars stay scary all game without retuning:
...
Why this works: the color faces hit harder the more you've invested in that color — a forged double-red build dreads the red face at −6+ while a balanced build shrugs at −3. Scars punish specialization, which gives **Poison a targeting logic** (hit the guy with the concentrated build, not just the leader) and makes every scarred player's roll a small drama the whole table watches. Counterplay is unchanged: reroll the Scar section (🎲/1⚡) or heal for 3⚡ — both now feel much more worth it.

⚠ Expected pain per scar roll roughly doubles vs v4 (~3+ per round mid-game vs ~1.5). Intended — scars should be hated — but sim the scar-count death-spiral case (3+ scars on last place) to confirm underdog ⚡ keeps them in the fight.
...
| **Contribution ladder** (replaces MVP): top three crew totals | **+5 / +3 / +1** | **+6 / +4 / +2** |
| Category win (best single die per color, crew only) | +2 (+1⚡) | +3 (+1⚡) |
| Host | 0 | 0 |

Host wins: **8 pts** early / **10 pts** late, +2⚡ (unchanged).
...
## 6. The host turn — Channels & the parasite shop (simmed ✅)

Replaces Feed / Harden / Outbreak / Poison entirely. Two ideas: **the parasite shops like a human**, and **your attack channel decides your collateral damage**.

### The parasite shop

- The parasite has its own Energy: **stipend 7⚡ per host turn** (6p) + whatever past hosts banked. Separate from your human ⚡.
- Same shop, same prices as the crew: **extra die 4⚡ · size-up 3⚡ (cap d10) · forge 1⚡ (+2 a face)** — nothing new to learn.
- **You must buy at least one thing.** The parasite always grows. Every purchase is permanent and shared with every future host.
- Leftover ⚡ banks on the creature (or pays parasite rerolls).

### The Channels

You roll your human dice first, **then** choose what the parasite drinks. The channel sets tonight's attack *and* the collateral:

| Channel | Tonight | Scars created |
|---|---|---|
| 🔴 **RAVAGE** (Weapons) | Parasite adds **2× your red total** | **Everyone** — all crew *and* you. Reckless. |
| 🔵 **CRUSH** (Strength) | Parasite adds **1.75× your blue total** | **2** — you + one crewmate you choose (2+-scar guardrail applies) |
| 🟢 **SEVER** (Agility) | **Cancels the crew's 2 highest dice** this round. No bonus to the parasite's own roll | **1** — you only |

Simmed mix at these numbers: **Sever 46% · Ravage 27% · Crush 27%** — no auto-pick. (First draft had Sever canceling 3 dice at 1.5× Crush: Sever was picked 88% of the time. The power ladder must match the collateral ladder — most scars buys most power.)

- The host **always** takes a scar — hosting costs a piece of you, every time.
- Channels replace the old human-share fusion. Parasite total = **Mirror base + parasite dice + Instinct die + channel effect**.
- **Boss Meter = scars created + host wins.** A Ravage-happy table builds itself a monstrous round 13.
...
Host turn sequence: human shop → parasite shop → roll human → **pick channel** → roll parasite (rerolls off parasite ⚡) → scars applied → submit both.

Sim results at these numbers: host ~48%, boss clear ~56% (boss HP ≈ **0.92× crew strength + 1.5× meter**), scar economy heavy but healthy — ~31 scars created per game, ~5 taken per player lifetime, only 17% of players ever hit 3+ concurrent, and strategy balance holds (5 of 6 bots in the 13–30% win band).

Open playtest questions:
- Do Sever-canceled dice also lose **category** eligibility? (Vicious and political — probably yes.)
- Can your second host turn repeat your first channel, or does the parasite crave variety?
- ⚠ Watch scar volume at the table: ~31/game is a lot of heal-tax and misery. If real players hate it more than sim bots do, first lever is Ravage scarring **3 chosen crew** instead of all 5.

## 6b. Parasite — the Mirror base (simmed ✅)
...
**New rule: the parasite's Base Attack mirrors the crew.**

> **Base Attack = ~48% of the crew's previous-round combined total** (seed ~55 for round 1). Crew holds still bump it +2 permanently.
...
- **HP also mirrors:** fixed HP numbers break whenever the economy changes. Set **HP ≈ 105% of the round-12 crew total + 1.5 × Boss Meter**. Sim calibration: ~50% clear rate, meter averaging ~27 after 12 host turns.

## 8. Sim findings (tools/v5-sim.mjs · 400 games · full results in artifacts/balance-v5.md)
...
**Scar death-spiral — real but self-inflicted.** Players who hit 3+ scars win ~0% and average 4.8th place — but almost nobody gets there unless they *refuse to heal* (the ignore-scars bot finished dead last at 4.8 avg; the heal-immediately bot was the strongest strategy in the sim). Verdict: heal 3⚡ is priced right, and "neglecting scars loses games" is the correct lesson. One guardrail worth adding: **Poison cannot target a player who already has 2+ scars** — stops pile-on griefing without softening scars themselves.
...
## 9. Superseded by the Channel redesign (§6)

- ~~Four host moves (Feed / Harden / Outbreak / Poison)~~ → Channels + mandatory scars
- ~~Human-share fusion (~50–62% of host attack)~~ → channel effects use your *actual rolled dice* instead
- ~~Poison as an optional move~~ → giving a scar is now automatic every host turn (Crush chooses the target; Ravage hits everyone)

## 10. Unchanged (deliberately)

- Scar cap 6 and heal price 3⚡ (faces redesigned — see §4)
...
5. New scar faces + Poison guardrail (no targets with 2+ scars)
6. Contribution ladder scoring
7. Parasite Mirror base (~48% of prev crew Σ) + mirrored boss HP
